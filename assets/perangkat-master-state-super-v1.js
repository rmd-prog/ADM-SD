...
function selectedDpl(mapel,old){const nodes=[...document.querySelectorAll('[data-dpl],input[name="dpl"],input[name="dpl[]"],select[name="dpl"],select[name="dpl[]"]')];const picked=nodes.filter(x=>x.checked||x.selected).map(x=>String(x.value||x.dataset.dpl||x.textContent||'').trim()).filter(x=>DPL.includes(x));if(picked.length)return [...new Set(picked)].slice(0,8);return Array.isArray(old?.dpl)&&old.dpl.length?old.dpl:defaultDpl(mapel)}
function selectedModel(old){const nodes=['modelPembelajaran','model','pkModel','modelPembelajaranSelect'].map($).filter(Boolean);for(const el of nodes){const v=el.value||el.textContent;if(v&&String(v).trim())return String(v).trim()}return old?.modelPembelajaran||'Problem Based Learning'}
function fromDom(){const c=controls();if(!c.rombel||!c.mapel||!c.bab)return null;const opt=$('pkB')?.options?.[$('pkB').selectedIndex];const text=(opt?.textContent||'').trim();const bm=text.match(/BAB\s*(\d+)\s*[—-]\s*(.+?)(?:\s*\((\d+)\s*JP\))?$/i);const babNo=bm?Number(bm[1]):Number(c.bab)||0;const material=bm?bm[2].trim():text.replace(/^BAB\s*\d+\s*[—-]\s*/i,'').replace(/\s*\(\d+\s*JP\).*$/i,'').trim();const jp=bm&&bm[3]?Number(bm[3]):Number(c.info?.textContent?.match(/(\d+)\s*JP/i)?.[1])||2;const semester=Number(c.info?.textContent?.match(/Semester\s*(\d+)/i)?.[1])||(babNo<=3?1:2);const lis=c.info?[...c.info.querySelectorAll('li')].map(x=>x.textContent.trim()).filter(Boolean):[];const old=read(KEY,null);const tp=lis.length?lis.map((x,i)=>({no:i+1,text:x,jp:0})):Array.from({length:4},(_,i)=>({no:i+1,text:'Peserta didik mampu '+['memahami konsep penting','mengidentifikasi dan menjelaskan informasi','menerapkan konsep dalam aktivitas atau pemecahan masalah','mengomunikasikan hasil dan melakukan refleksi'][i]+' pada materi '+material+'.',jp:0}));return {source:'GURU_SD_MASTER',rombel:c.rombel,mapel:c.mapel,babId:c.rombel+'|'+c.mapel+'|'+babNo,babNo,material,semester,jp,tp,dpl:selectedDpl(c.mapel,old),modelPembelajaran:selectedModel(old),fase:phase(c.rombel),tahunPelajaran:'2026/2027',updatedAt:new Date().toISOString()};}
function save(x){if(!x)return null;const clean={...x,source:'GURU_SD_MASTER',tp:Array.isArray(x.tp)?x.tp:[],dpl:Array.isArray(x.dpl)?x.dpl:[],updatedAt:x.updatedAt||new Date().toISOString()};localStorage.setItem(KEY,JSON.stringify(clean));localStorage.setItem(LEGACY,JSON.stringify(clean));window.dispatchEvent(new CustomEvent('guruSdMasterChanged',{detail:clean}));return clean;}
function sync(){const x=fromDom();if(!x)return null;const prev=read(KEY,null);if(prev&&prev.babId===x.babId&&prev.material===x.material&&prev.jp===x.jp){x.modelPembelajaran=selectedModel(prev);x.dpl=selectedDpl(x.mapel,prev);}return save(x);}
function get(){const dom=fromDom();if(dom)return sync();const x=read(KEY,null);if(x)return x;return {source:'GURU_SD_MASTER',rombel:'',mapel:'',babId:'',babNo:0,material:'',semester:1,jp:0,tp:[],dpl:DPL,modelPembelajaran:'Problem Based Learning',fase:'A',tahunPelajaran:'2026/2027'};}
function set(patch){return save({...get(),...patch,updatedAt:new Date().toISOString()});}
window.GURU_SD_MASTER={KEY,get,set,sync,save,phase,dpl:defaultDpl,esc};
