/* SIAP GURU — Rekap Nilai Smart V2
...
function render(force){
 ...
 const index={};
 rows.forEach(x=>{
   if(!x||String(x.key||'').endsWith('|abs'))return;
   const jenis=String(x.jenis||'').toLowerCase(),id=String(x.studentId??'');
   if(!id)return;
   const xm=String(x.mapel||''),xs=String(x.semester||'');
   if(xm!==m||xs!==sem)return;
   const g=index[id]??={formative:[],task:[],chapter:{},summative:[]};
   const v=Number(x.nilai);if(!Number.isFinite(v))return;
   if(jenis.includes('formatif'))g.formative.push(v);
   else if(jenis.includes('tugas'))g.task.push(v);
   else if(jenis.includes('bab'))g.chapter[String(x.item||'')]=v;
   else if(jenis.includes('sumatif'))g.summative.push(v);
 });
 const out=ss.map(s=>{
   const g=index[s.id]||{formative:[],task:[],chapter:{},summative:[]};
   const f=avg(g.formative),t=avg(g.task),cvals=ch.map(b=>g.chapter[b]).filter(v=>Number.isFinite(v)),c=avg(cvals),su=avg(g.summative);
   const parts=[[f,w.formative],[t,w.task],[c,w.chapter],[su,w.summative]].filter(([v,b])=>v!==null&&b>0);const denom=parts.reduce((a,[,b])=>a+b,0);const final=denom?Math.round(parts.reduce((a,[v,b])=>a+v*b,0)/denom):0;
   return {...s,g,f,t,c,su,final};
 });
 const mean=a=>a.length?Math.round(a.reduce((x,y)=>x+y,0)/a.length):0;
 const fAll=out.filter(x=>x.f!==null).map(x=>x.f),tAll=out.filter(x=>x.t!==null).map(x=>x.t),cAll=out.filter(x=>x.c!==null).map(x=>x.c),sAll=out.filter(x=>x.su!==null).map(x=>x.su),finalAll=out.filter(x=>x.final>0).map(x=>x.final);
