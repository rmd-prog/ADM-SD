/* GURU+ SD — MASTER ENFORCER + CHAIN GUARD V6
 ...
 * Active document context, including RPM, is always synchronized to the persisted Master.
 * No D1/Worker access.
 */
(function(){'use strict';
if(window.__GURU_SD_MASTER_ENFORCER_V6__)return;window.__GURU_SD_MASTER_ENFORCER_V6__=1;
...
function selectByMaster(select,x){
  if(!select||!x)return;
  const opts=[...select.options];
  let i=opts.findIndex(o=>String(o.textContent||'').trim().toLowerCase().includes(String(x.material||'').trim().toLowerCase()));
...
function syncDom(x){
  if(!x)return;
  const r=document.getElementById('pkR'),m=document.getElementById('pkM'),b=document.getElementById('pkB'),rpm=document.getElementById('rpmBab');
  if(r&&x.rombel&&r.value!==x.rombel)r.value=x.rombel;
  if(m&&x.mapel&&m.value!==x.mapel)m.value=x.mapel;
  if(b&&x.material)selectByMaster(b,x);
  if(rpm&&x.material)selectByMaster(rpm,x);
}
...
function patchGenerateDoc(){
  ...
  async function guarded(type){const x=master();if(!x||!x.rombel||!x.mapel||!x.babId)throw new Error('MASTER_STATE_NOT_READY');syncDom(x);window.dispatchEvent(new CustomEvent('guruSdMasterBeforeGenerate',{detail:x}));return original.apply(this,arguments)}
