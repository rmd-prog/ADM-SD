/* ADM-SD — role menu + AI Brain V4.0.1 loader */
(function(){
'use strict';
...
function applyRoleMenu(){
 ...
 document.querySelectorAll('.menu-group').forEach(g=>{if(admin){g.style.display='';return}const visible=[...g.querySelectorAll('.navbtn')].some(x=>getComputedStyle(x).display!=='none');const title=norm(g.querySelector('.navgroup')?.textContent);if(title.includes('sistem')||title.includes('referensi sibi'))g.style.display='none';else if(g.querySelector('.submenu')&&!visible)g.style.display='none'});
...
function boot(){applyRoleMenu();loadScript('assets/multi-rombel.js?v=1','__ADM_MULTI_ROMBEL_LOADED');loadScript('assets/ai-brain-v40.js?v=401','__ADM_AI_V40_LOADED');window.__ADM_APPLY_ROLE_MENU=applyRoleMenu}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
const obs=new MutationObserver(()=>{if(!window.__ADM_AI_V40_LOADED)loadScript('assets/ai-brain-v40.js?v=401','__ADM_AI_V40_LOADED')});
