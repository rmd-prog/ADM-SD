/* GURU+ SD — MASTER ENFORCER + CHAIN GUARD V7
 ...
 */
(function(){'use strict';
if(window.__GURU_SD_MASTER_ENFORCER_V7__)return;window.__GURU_SD_MASTER_ENFORCER_V7__=1;
...
function syncDom(x){if(!x)return;const r=document.getElementById('pkR'),m=document.getElementById('pkM'),b=document.getElementById('pkB'),rpm=document.getElementById('rpmBab'),ai=document.getElementById('aiRpmBab');if(r&&x.rombel&&r.value!==x.rombel)r.value=x.rombel;if(m&&x.mapel&&m.value!==x.mapel)m.value=x.mapel;if(b&&x.material)selectByMaster(b,x);if(rpm&&x.material)selectByMaster(rpm,x);if(ai&&x.material)selectByMaster(ai,x)}
...
function boot(){guardMirrorSelectors();audit();window.addEventListener('guruSdMasterChanged',e=>{mirror(e.detail);setTimeout(audit,0)});['pkR','pkM','pkB','pkI','rpmBab','aiRpmBab'].forEach(id=>{const el=document.getElementById(id);if(el)el.addEventListener('change',()=>setTimeout(audit,0))});setInterval(audit,2000)}
