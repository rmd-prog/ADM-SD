/* ADM-SD — GLOBAL APP INTEGRITY SUPER v1 */
(function(){
'use strict';
...
function dedupeNav(){const side=q('.side');if(!side)return;const seen=new Set();qa('.navbtn,.navgroup',side).forEach(el=>{const key=(el.id||'')+'|'+(el.getAttribute('onclick')||'')+'|'+(el.textContent||'').replace(/\s+/g,' ').trim();if(!key||key==='|')return;if(seen.has(key)){el.remove();return}seen.add(key)})}
...
window.GURU_SD_GLOBAL_APP_INTEGRITY={repair,version:'v1'};
