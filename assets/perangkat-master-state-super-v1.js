/* GURU+ SD — SINGLE SOURCE OF TRUTH V3
 * One canonical state for BAB -> CP -> ATP -> TP -> JP -> Jadwal -> PROTA -> PROMES -> RPM -> LKPD -> Asesmen.
 * UI BAB option is authoritative for identity. Informational #pkI is never allowed to leak stale TP/semester into a new BAB.
 * All document generators consume GURU_SD_MASTER.get().
 * Local-only. Never touches D1, Worker, students, login, or assessment bridge.
 */
(function(){'use strict';
if(window.__GURU_SD_MASTER_STATE_V3__)return;window.__GURU_SD_MASTER_STATE_V3__=1;
...
function fromDom(){const c=controls();if(!c.rombel||!c.mapel||!c.bab)return null;const p=parseBab();if(!p.material)return null;const old=read(KEY,null);const babId=c.rombel+'|'+c.mapel+'|'+p.babNo;const sameContext=!!old&&old.rombel===c.rombel&&old.mapel===c.mapel&&old.babId===babId;
  /* IMPORTANT: #pkI is informational UI and may lag behind #pkB. Never read its TP or semester here. */
  const tp=fallbackTp(p.material);
...
function get(){const dom=fromDom();if(dom)return dom;const x=read(KEY,null);if(x)return x;return {source:'GURU_SD_MASTER',rombel:'',mapel:'',babId:'',babNo:0,material:'',semester:1,jp:0,tp:[],dpl:DPL,modelPembelajaran:'Problem Based Learning',fase:'A',tahunPelajaran:'2026/2027'};}
