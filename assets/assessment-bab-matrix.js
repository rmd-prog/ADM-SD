/* SIAP GURU — Input Nilai BAB Matrix V3
   One persistent table: students x BAB. Scroll-safe rendering.
...
function students(){const a=window.db?.students||window.students||[];const r=rombel();return Array.isArray(a)?a.map((s,i)=>({id:String(s.id??s.nisn??s.nis??i),name:String(s.name??s.nama??s.nama_siswa??'').trim(),rombel:normR(s.rombel??s.kelas??s.rombel_siswa??'')})).filter(s=>s.name&&(!r||r==='ALL'||!s.rombel||s.rombel===r)):[]}
...
function render(force){if(!active())return;const host=$('#sgAssessmentBody');if(!host)return;const oldWrap=host.querySelector('.tablewrap');const oldX=oldWrap?.scrollLeft||0,oldY=oldWrap?.scrollTop||0;const d=read(KEY,{weights:{formative:20,task:20,chapter:30,summative:30},rows:{}}),ss=students(),ms=mapels();let m=host.dataset.babMapel||ms[0]||'',sem=host.dataset.babSem||'1',edit=host.dataset.babEdit==='1';if(!ms.includes(m))m=ms[0]||'';const ch=chapters(m),sig=[m,sem,ss.length,edit].join('|');if(!force&&host.dataset.babSig===sig&&host.querySelector('#sgBabMatrix'))return;host.dataset.babSig=sig;host.dataset.babMapel=m;host.dataset.babSem=sem;host.dataset.babEdit=edit?'1':'0';
const vals=(sid,b)=>{const row=d.rows?.[`${sid}|${m}|${sem}|Ulangan BAB|${b}`];return row?.nilai??''};
...
if(edit)$('#sgBabSave').onclick=()=>{const now=read(KEY,{weights:{formative:20,task:20,chapter:30,summative:30},rows:{}});now.rows=now.rows||{};let n=0;$$('#sgBabMatrix .sgBabScore').forEach(i=>{const sid=i.dataset.sid,b=i.dataset.bab,v=i.value.trim(),key=`${sid}|${m}|${sem}|Ulangan BAB|${b}`;if(v==='')return;now.rows[key]={studentId:sid,mapel:m,semester:sem,jenis:'Ulangan BAB',item:b,nilai:Math.max(0,Math.min(100,Number(v))),catatan:now.rows[key]?.catatan||''};n++});write(KEY,now);host.dataset.babEdit='0';host.dataset.babSig='';render(true);document.dispatchEvent(new Event('adm:assessment-saved'));window.ADM_UI?.success?.(`${n} nilai BAB disimpan.`)};
...
setInterval(()=>{wire();render(false)},1200);setTimeout(()=>{wire();render(true)},350);
})();
